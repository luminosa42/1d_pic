! Simple FFT-based 1D Poisson solver. Periodic boundary conditions.
! Paul Ricker 2/2016
...
! 1D Poisson solver routine. Solves the equation d^2(phi)/dx^2 = rho on
! a 1D Cartesian mesh with periodic boundary conditions.

! Arguments:
!   rho           (input)  Real array of length n containing the source density
!   phi           (output) Real array of length n containing the potential
!   n             (input)  Integer number of elements in rho and phi. Must be a
!                          power of 2 to work.
!   dx            (input)  Real zone spacing
!   iinit         (input)  Integer initialization switch. On calling this
!                          routine for the first time, or after changing the
!                          mesh spacing or number of zones, use a nonzero value
!                          for iinit. Thereafter use iinit = 0.

subroutine solve_poisson(rho, phi, n, dx, iinit)

!------------------------------------------------------------------------------

integer, intent(in)    :: n, iinit
real, intent(in)       :: dx, rho(0:n-1)
real, intent(out)      :: phi(0:n-1)

real, pointer, save    :: wk1(:), G(:)
integer, pointer, save :: ip(:)
real, save             :: norm, dxsave
integer, save          :: nsave
...
! Initialization and idiot-proofing

if (2**(log(float(n))/log(2.)) /= n) then
  write(*,*) '[solve_poisson] n must be a power of 2!'
  stop
endif

if (iinit /= 0) then
  if (associated(G))   deallocate(G)
  if (associated(wk1)) deallocate(wk1)
  if (associated(ip))  deallocate(ip)
  allocate(G(1:n/2))
  allocate(wk1(0:n*5/4-1))
  allocate(ip(0:1+int(sqrt(float(n/2)))))
  ip(0) = 0
  do i = 1, n/2
    G(i) = -0.5*dx**2 / (cos((2.*pi/n)*i)-1.)  ! 2nd order finite difference
  enddo
  norm = 2./(float(n))
  dxsave = dx
  nsave = n
else
  if ((.not. associated(G)) .or. (nsave /= n) .or. &
      (abs(dxsave-dx) > tiny(1.))) then
    write(*,*) '[solve_poisson] must call with iinit = 1 first!'
    stop
  endif
endif

! Do not overwrite input density

phi = rho

! Forward transform

call rdft (n, 1, phi, ip, wk1)

! Multiply by Green's function

do i = 1, n/2-1
  phi(2*i  ) = phi(2*i  ) * G(i)
  phi(2*i+1) = phi(2*i+1) * G(i)
enddo

phi(0) = 0.
phi(1) = phi(1) * G(n/2)

! Inverse transform and apply normalization
